...
nyoka = ssl.create_default_context()
nyoka.check_hostname = False
nyoka.verify_mode = ssl.CERT_NONE

url = input('Enter url please: ')
position = int(input("Enter position:"))-1
count = int(input("Enter count:"))
html = urllib.request.urlopen(url, context=nyoka).read()
...
for i in range(count):
    ...
    html = urllib.request.urlopen(link, context=nyoka).read()
    soup = BeautifulSoup(html, 'html.parser')
    tags = soup('a')
    link = tags[position].get('href', None)
print(Sequence[-1])
